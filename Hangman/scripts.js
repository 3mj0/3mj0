...
function startGame() {
    ...
    displayWord();
    clearCanvas();
    clearWrongLetters(); // Clear wrong letters display
}
...
function handleGuess(event) {
    ...
    if (selectedWord.includes(letter)) {
        ...
        wrongGuesses++;
        displayWrongLetters(letter); // Update wrong letters display
        drawHangman();
        checkLose();
    }
}
...
function displayWrongLetters(letter) {
    ...
    const currentText = wrongLettersDiv.innerHTML;
    
    // Check if the letter is already displayed
    if (!currentText.includes(letter)) {
        wrongLettersDiv.innerHTML = currentText + (currentText === 'Wrong Letters: ' ? '' : ', ') + letter;
    }
}
